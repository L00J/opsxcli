...
func (e *EvolverEngine) Evolve(ctx context.Context, exec *TaskExecution) *EvolveResult {
	...
	newExp := e.stepLearn(exec, observation, keyDecisions, toolScores, similar)
...
func (e *EvolverEngine) stepLearn(exec *TaskExecution, observation map[string]interface{}, decisions []string, scores map[string]float64, similar *Experience) *Experience {
	taskType := e.classifyTaskType(exec.Query)

	// 生成优化提示
	hint := e.generateHint(exec, observation, decisions)
...
// generateHint 生成优化提示
func (e *EvolverEngine) generateHint(exec *TaskExecution, observation map[string]interface{}, decisions []string) string {
	hints := make([]string, 0)
