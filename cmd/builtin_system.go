...
func NewFreeCmd() *cobra.Command {
	var opts builtin.FreeOptions
	cmd := &cobra.Command{
		Use:   "free [flags]",
		Short: "显示内存使用情况",
		Long:  "显示系统物理内存和交换分区使用情况（Go 原生实现，跨平台兼容）",
		RunE: func(cmd *cobra.Command, args []string) error {
			return builtin.Free(opts)
		},
	}
...
func NewDfCmd() *cobra.Command {
	var opts builtin.DfOptions
	cmd := &cobra.Command{
		Use:   "df [flags] [path...]",
		Short: "显示磁盘空间使用情况",
		Long:  "显示文件系统磁盘空间使用情况（Go 原生实现，跨平台兼容）",
		RunE: func(cmd *cobra.Command, args []string) error {
			return builtin.Df(opts)
		},
	}
	cmd.Flags().BoolVarP(&opts.Human, "human", "h", false, "人类可读的大小 (如 1.2Gi)")
